...
const updateProfile = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = await technicianService.updateTechnicianProfileDb(userId, req.body)
...
const updateBookingStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const allowedStatus: BookingStatus[] = ["accepted", "declined", "completed"]
        if (!status || !allowedStatus.includes(status)) {
            throw new AppError(400, "Invalid status. Allowed: accepted, declined, completed")
        }

        const result = await technicianService.updateBookingStatusDb(userId, id as string, status)
